...
class InstructionDecoderUnitTest {
    ...
    fun `Decode shift right instruction (SHR Vx {, Vy})`(param: Int) {
        expectThat(instructionDecoder.decode(param.toUInt())).isEqualTo(Cpu::shiftRightXOnlyVariant)
    }
    ...
    fun `Decode add I register to general register value instruction (ADD I, Vx)`(param: Int) {
        expectThat(instructionDecoder.decode(param.toUInt())).isEqualTo(Cpu::addGeneralRegisterToIRegister)
    }
